...
async def nfc_check_in(
    ...
    patient_result = await db.execute(
        select(User).where(User.id == card.patient_id)
    )
    ...
    if not queue:
        logger.error("No active queue found", extra={"practice_id": str(device.practice_id)})
        return NFCCheckInResponse(
            success=False,
    ...
    if existing_ticket:
        return NFCCheckInResponse(
            ...
            estimated_wait_minutes=existing_ticket.estimated_wait_time,
    ...
    try:
        ...
        await notify_check_in_success(
            ...
            ticket_number=ticket_number,
            queue_name=queue.name,
            estimated_wait_minutes=ticket.estimated_wait_time,
        )
...
async def get_patient_cards(
    ...
    return NFCCardListResponse(
        items=[NFCCardResponse.model_validate(c) for c in cards],
        total=len(cards)
    )
...
async def get_check_in_history(
    ...
    method: Optional[CheckInMethod] = Query(None, description="Filter by check-in method"),
) -> CheckInEventListResponse:
    """
    ...
    for event in events:
        ...
        response_items.append(CheckInEventResponse(
            id=event.id,
