...
def upgrade() -> None:
    # Create ENUM types
    op.execute(
        "CREATE TYPE userrole AS ENUM ('admin', 'doctor', 'mfa', 'staff', 'patient')"
    )
    op.execute(
        "CREATE TYPE ticketstatus AS ENUM ('waiting', 'called', 'in_progress', 'completed', 'cancelled', 'no_show')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "max_daily_tickets", sa.Integer(), nullable=False, server_default="100"
        ),
        sa.Column(
    ...
    op.create_table(
        ...
        sa.Column(
            "message_type", sa.String(50), nullable=False, server_default="'text'"
        ),
    ...
    op.create_index(
        "ix_chat_messages_room_created", "chat_messages", ["room_id", "created_at"]
    )
