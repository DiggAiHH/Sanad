...
async def list_devices(
    ...
    return IoTDeviceListResponse(
        items=[IoTDeviceResponse.model_validate(d) for d in devices], total=len(devices)
    )
...
async def register_device(
    ...
    if not practice:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Keine Praxis konfiguriert"
        )
...
async def update_device(
    ...
    if not device:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Gerät nicht gefunden"
        )
...
async def list_zones(
    ...
    result = await db.execute(
        select(Zone).where(Zone.is_active.is_(True)).order_by(Zone.zone_name)
    )
    zones = list(result.scalars().all())

    return ZoneListResponse(
        items=[ZoneResponse.model_validate(z) for z in zones], total=len(zones)
    )
...
async def create_route(
    ...
    route = WayfindingRoute(
        ...
        led_color=request.route_color,
        led_segment_ids=json.dumps([str(seg_id) for seg_id in request.segment_ids]),
        duration_seconds=request.animation_speed,
    )
...
async def trigger_route(
    ...
    if not route:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Route nicht gefunden"
        )
...
async def get_wait_time_overview(
    ...
    result = await db.execute(
        select(Zone).where(Zone.is_active.is_(True)).where(Zone.zone_type == "waiting")
    )
    zones = list(result.scalars().all())
    ...
    for zone in zones:
        ...
        if tickets:
            ...
            for ticket in tickets:
                wait_minutes = int(
                    (datetime.now(timezone.utc) - ticket.created_at).total_seconds()
                    / 60
                )
